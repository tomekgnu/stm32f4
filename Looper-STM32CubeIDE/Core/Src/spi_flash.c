...
uint16_t sFLASH_SendHalfWord(uint16_t HalfWord)
{
  ...
  uint8_t tran[2],recv[2];
  tran[0] = (uint8_t)(HalfWord >> 8);
  tran[1] = (uint8_t)HalfWord;
  
  HAL_SPI_TransmitReceive(&hspi5,tran,recv,2,100);

  HalfWord |= (((uint16_t)recv[0]) << 8);
  HalfWord |= (uint16_t)recv[1];
