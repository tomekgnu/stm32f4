...
void SRAM_writeSingleTrack(__IO CHANNEL *ch){
	uint32_t allbytes = (GET_ACTIVE_CHANNEL)->SamplesWritten * 2;
...
void SD_readToSDRAM(FIL *fp){
	...
	while(1){
		...
		(GET_ACTIVE_CHANNEL)->SamplesWritten += (bytes_read / 2);
...
void SD_writeSingleTrack(__IO CHANNEL *ch,FIL *fp){
	uint8_t *_buf;
	uint32_t bytesRemaining = (GET_ACTIVE_CHANNEL)->SamplesWritten * 2;
	_buf = (uint8_t *)malloc(8192);
	bytes_written = 0;
	sdram_pointer = 0;
	f_truncate(fp);
